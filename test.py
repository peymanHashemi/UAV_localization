...
parser.add_argument('--seg_test_dir', default='', type=str, help='./test_segment')
...
seg_image_datasets_query = {x: datasets.ImageFolder(os.path.join(
    seg_test_dir, x), seg_data_query_transforms) for x in ['seg_query_drone']}

seg_image_datasets_gallery = {x: datasets.ImageFolder(os.path.join(
    seg_test_dir, x), seg_data_transforms) for x in ['seg_gallery_satellite']}
