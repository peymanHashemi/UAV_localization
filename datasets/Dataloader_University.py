...
class Dataloader_University(Dataset):
    def __init__(self, root, transforms, names=['satellite', 'drone'],seg_root=''):
        ...
        self.seg_transforms_drone_street = transforms['seg_train']
        self.seg_transforms_satellite = transforms['seg_satellite']
        self.root = root
        ...
        if seg_root:
            ...
            for name in names:
                dict_ = {}
                for cls_name in os.listdir(os.path.join(seg_root, name)):
                    img_list = os.listdir(os.path.join(seg_root, name, cls_name))
                    img_path_list = [os.path.join(
                        seg_root, name, cls_name, img) for img in img_list]
                    dict_[cls_name] = img_path_list
                seg_dict_path[name] = dict_
            ...
            self.seg_dict_path = seg_dict_path
    ...
    # 从对应的类别中抽一张出来
    def sample_from_cls(self, name, cls_num, seg=False):

        if seg:
            img_path = self.seg_dict_path[name][cls_num]
    ...
    def __getitem__(self, index):
        ...
        img = self.sample_from_cls("satellite", cls_nums,seg =True)
        seg_s = self.seg_transforms_satellite(img)
        
        img = self.sample_from_cls("drone", cls_nums, seg =True)
        seg_d = self.seg_transforms_drone_street(img)

        
        return img_s, seg_s, img_d, seg_d, index
...
def train_collate_fn(batch):
    ...
    """
    img_s, img_d, seg_s, seg_d, ids = zip(*batch)
    ids = torch.tensor(ids, dtype=torch.int64)
    return [torch.stack(img_s, dim=0), ids], [torch.stack(img_d, dim=0), ids], [torch.stack(seg_s, dim=0), ids], [torch.stack(seg_d, dim=0), ids]
